function $_global_init() {
    ...
    XUIHtml.SetOpacity = function(a, b) {
        ...
        if (document.body.style.opacity != null)
            if (b == 1) XUIHtml.RemoveCSSProperty(a, "opacity");
    ...
    try {
        if (typeof window.localStorage != "undefined") {
            ...
            if (TestStorage(b)) _local = b
        }
    } catch (f) {}
    try {
        if (typeof window.sessionStorage != "undefined") {
            var a = window.sessionStorage;
            if (TestStorage(a)) _session = a
        }
    } catch (e) {}
    if (_session == null) _session = new StorageObject;
    if (_local == null) _local = _session;
    ...
    if (!window.location.pathname.toLowerCase().endsWith("/_layouts/15/start.aspx")) {
        ...
        if (Boolean(document.addEventListener)) document.addEventListener("DOMContentLoaded", c, false);
        else _spBodyOnLoadFunctions.push(d)
    }
    URI = function(M, d) {
        var f = this,
            v = "#",
        ...
        this.getPath = function(b) {
            ...
            if (typeof b === "boolean" && b)
                if (a !== null && a.lastIndexOf(i) === a.length - 1) a = a.slice(0, -1);
            return a
        };
        ...
        this.getQueryParameter = function(d) {
            var a = null,
                b = f.getQueryAsObject();
            if (n) {
                for (var c in b)
                    if (h.hasOwnProperty(c) && c.toLowerCase() === d.toLowerCase()) a = b[c]
            } else a = b[d];
        ...
        var A = function(a) {
            ...
            t = function(e) {
                ...
                l = b(e);
                for (var a = e.split(i), d = 0; d < a.length; ++d) {
                    var m = a[d],
                        f = m.split(o),
                        k = b(f[0]);
                    a[d] = c(k);
...
function GoToLink(c) {
    ...
    if (b)
        if (c.target === "_blank") window.open(a, "_blank");
...
function ShowAttachmentRows() {
    ...
    if (a != null)
        if (b == null || b.rows.length == 0) a.style.display = "none";
...
function SP_JSONParse(f) {
    ...
    function e(c, b) {
        a = a || typeof b === "undefined" && /^\d+$/.test(c)
    }
}
...
function UpdateFormDigest(f, i) {
    try {
        ...
        var g = "<GetUpdatedFormDigestResult>",
            j = "</GetUpdatedFormDigestResult>",
            e = c.indexOf(g),
            h = c.indexOf(j, e + g.length),
            d = null;
...
function GetStssyncData(d, g, e, h) {
    ...
    if (document.cookie.indexOf("stsSyncAppName") == -1 && document.cookie.indexOf("stsSyncIconPath") == -1) {
        ...
        if (browseris.ie5up && browseris.win32 || IsSupportedMacBrowser()) {
            ...
            document.cookie = "stsSyncAppName=" + escapeProperly(b) + ";path=/";
            try {
                if (typeof g_objStssync.GetStssyncIconName == "undefined") throw 0;
                c = g_objStssync.GetStssyncIconName();
                c = h + c;
                document.cookie = "stsSyncIconPath=" + escapeProperly(c) + ";path=/"
            } catch (i) {
                document.cookie = "stsSyncIconPath=0;path=/";
                c = e
            }
            ...
            b = g;
            c = e;
            document.cookie = "stsSyncAppName=" + escapeProperly(g) + ";path=/";
            document.cookie = "stsSyncIconPath=" + escapeProperly(e) + ";path=/"
        }
    } else {
...
function GetThemedImageUrl_Core(e, g) {
    var f = g + e,
        c = window._spPageContextInfo;
    if (c != null) {
        var b = c.themedCssFolderUrl,
            d = c.themedImageFileNames,
            a = null;
        if (d != null) a = d[e];
...
var CTXTYPE_EDITMENU, CTXTYPE_VIEWSELECTOR;
...
function GetSecuredSource(b) {
    var a = DeferCall("GetSource2", b, null);
    if (a == null) {
        var c = GetUrlKeyValue("Source");
        if (c == "")
            if (b != null && b != "") c = b;
            else c = ajaxNavigate.get_href();
        a = c
    }
    if (a != b && a != ajaxNavigate.get_href()) {
        var e = new URI(ajaxNavigate.get_href()),
            f = new URI(a),
            d = f.getAuthority(),
            g = d == null || d == "";
        if (!g && (e.getScheme().toLowerCase() !== f.getScheme().toLowerCase() || e.getAuthority().toLowerCase() !== d.toLowerCase())) a = b
    }
    return escapeProperly(a)
}
...
function STSNavigate(a) {
    ...
    if (window.frameElement != null || typeof SPUpdatePage === "undefined" || typeof SPUpdatePage !== "undefined" && SPUpdatePage(a))
        if (isPortalTemplatePage(a)) window.top.location.href = STSPageUrlValidation(a);
        else window.location.href = STSPageUrlValidation(a)
}
...
function ExpCollGroup(c, F, y, w) {
    ...
    var H = d.src,
        ...
        B = fRightToLeft ? "ms-commentcollapsertl-icon" : "ms-commentcollapse-icon",
        A = fRightToLeft ? "ms-commentcollapsertl-iconouter" : "ms-commentcollapse-iconouter",
        q = fRightToLeft ? "ms-commentexpandrtl-icon" : "ms-commentexpand-icon",
        t = fRightToLeft ? "ms-commentexpandrtl-iconouter" : "ms-commentexpand-iconouter",
        p = "/_layouts/15/images/plus.gif",
        z = "/_layouts/15/images/minus.gif",
        e = false,
        h;
    if (o == p || o == l && d.className == q || g_ExpInitializing) {
        e = true;
        h = "";
        d.alt = Strings.STS.L_SPCollapse;
        if (o == l) {
            d.className = B;
    ...
    for (var f = 0, s = 0; s < G; s++) {
        ...
        if (a.id != null && a.id.length > r + 4 && c == a.id.slice(4).substr(0, r)) {
            ...
            if (typeof FV4UI != "undefined" && FV4UI() && Boolean(m))
                ...
                i.alt = Strings.STS.L_SPExpand;
                if (D == l) {
                    i.className = q;
                    i.parentNode.className = t
                } else i.src = p
            }
            var v = "tbod" + c,
                j;
...
function LoadSodInternal(a, e) {
    ...
    var k = [],
        i = a.depkeys;
    if (i != null) {
        for (var q = true, m = i.length, b, c = 0; c < m; c++) {
            b = _v_dictSod[i[c]];
            if (b == null) continue;
            if (!IsSodLoaded(b)) {
                q = false;
    ...
    if (e) {
        ...
        var p = false,
            ...
            o = v.getElementsByTagName("script"),
            t = o.length,
...
function FixRibbonAndWorkspaceDimensions() {
    ...
    var o = GetCachedElement("s4-ribbonrow"),
        ...
        v = GetCachedElement("footer"),
        s = GetCachedElement("ms-core-overlay"),
        q = GetCachedElement("globalNavBox"),
        c;
    ...
    var g = 0,
        p = Boolean(ajaxNavigate.get_search().match(RegExp("[?&]IsDlg=1")));
    ...
    var r = GetCachedElement("s4-ribbonrow"),
        e = r.querySelector(".ms-core-webpartadder");
    ...
    if (!g_setScrollPos) {
        ...
        if (Boolean(ajaxNavigate.get_search().match(RegExp("[?&]IsDlg=1"))))
            if (!u || a.scrollHeight < a.clientHeight) a.style.overflowY = "auto";
...
function _registerCssLink(j, f, i, b) {
    ...
    for (var d = f.getElementsByTagName("link"), p = d.length, h = GetAbsoluteUrl(j), c = 0; c < p; c++)
        if (d[c].rel == "stylesheet" && d[c].type == "text/css") {
            var e = d[c].href;
        ...
        if (browseris.chrome || browseris.safari) {
            ...
            setTimeout(bindArguments(function m(c, a) {
                if (typeof a.sheet == "undefined" || a.sheet == null || typeof a.sheet.cssRules == "undefined" || a.sheet.cssRules == null)
                    if (Number(new Date) - c < o) setTimeout(bindArguments(m, c, a), g);
    ...
    if (document.forms.length > 0 && !Boolean(_spOriginalFormAction)) {
        ...
        if (b >= 0) {
            ...
            if (a.length > 2 && a.charAt(0) == "/" && a.charAt(1) == "/") {
                a = a.substring(1);
                while (a.charAt(1) === "/") a = a.charAt(0) + a.substring(2)
            }
        }
...
function ProcessDefaultOnLoad() {
    ...
    if (ajaxNavigate.get_hash().indexOf("InplviewHash") != -1) {
        ...
        var d = "inplview.RestoreAllClvpsNavigation",
            b = d.split(".");
        if (b.length > 1) {
            var c = function() {
...
function initPromotionBar(c) {
    var b = document.getElementById("DeltaPageStatusBar");
    if (b != null) {
        var a = document.createElement("div");
        a.id = "migrationToolPromotionBar";
        a.setAttribute("aria-live", "polite");
        a.setAttribute("aria-relevant", "all");
        a.className = "ms-status-green";
        a.style.display = "block";
        b.appendChild(a);
        var d = _createStatusMarkup("", c, false, false, true);
        a.appendChild(d)
    }
}

function _createStatusMarkup(e, c, h, d, f, g) {
    ...
    a.push("</span>");
    !d && a.push('<span class="ms-status-iconSpan"><img class="ms-status-iconImg" src="/_layouts/15/images/spcommon.png" /></span>');
    if (e.length != 0) {
    ...
    if (f) {
        ...
        a.push(b.id);
        if (String(c).toLowerCase().indexOf("configssc.aspx") != -1) a.push("');__doPostBack('DismissSiteCreationOnlineNotification');\"/></a>");
        else if (String(c).indexOf("2088320") != -1 || String(c).indexOf("2089901") != -1) a.push("');__doPostBack('DismissMigrationToolPromotionTip');\"/></a>");
        else a.push("');__doPostBack('DismissOffice365Notification');\"/></a>")
    }
    h && !d && a.push("<br/>");
    b.innerHTML = a.join("");
    ...
    if (a != null) {
        ...
        if (Boolean(b))
            if (b.getElementsByTagName("SPAN").length == 0) {
                ...
                StatusIdWithTopPriority = null;
                b.style.display = "none"
            } else if (e == StatusIdWithTopPriority) {
                _selectStatusWithTopPriority();
                b.className = StatusBarClassNames[StatusColorWithTopPriority]
            }
        var d = document.getElementById("migrationToolPromotionBar");
        if (Boolean(d))
            if (d.getElementsByTagName("SPAN").length == 0) d.style.display = "none"
    }
}
...
function RenderSuiteNav(b) {
    var k = b.top,
        a = b.dataEndpoint,
        d = b.culture,
        c = b.version;
    if (!(c > 0)) c = c_defaultSuiteDataVersion;
    if (!!a && !!d) a += (a.indexOf("?") < 0 ? "?" : "&") + "v=" + String(c) + "&Locale=" + d;
    var f = "",
        h = null,
        j = false;
    WithSuiteNavData(function(a) {
        h = a;
        (!j || Boolean(f)) && e(a)
    }, a, c);

    function e(d) {
        var a = window.O365Shell;
        if (a == null) return;
        var b = ExtractSuiteNavBarData(d);
        RemoveEmptyArraysFromSuiteNavData(b);
        a.SetData(b);
        var c = {
            top: k,
            layout: "Mouse"
...
function SuiteNavData() {
    ...
    a.HelpLink = new LinkData;
    a.SignOutLink = new LinkData
}
...
function RemoveEmptyArraysFromSuiteNavData(d) {
    for (var c = ["CurrentWorkloadHelpSubLinks", "CurrentWorkloadSettingsSubLinks", "CurrentWorkloadUserSubLinks", "PinnedApps", "WorkloadLinks"], e = c.length, a = 0; a < e; a++) {
        var b = c[a];
        if (IsEmptyArray(d[b])) d[b] = null
    }
}

function MakeSuiteNavDataLoader(a, b) {
    function c(d) {
        if (Boolean(a)) {
            var c = "/_api/" + a;
            WithRestValue(function(e) {
                var a;
                try {
                    var c = JSON.parse(e);
                    a = c.d.GetSuiteNavData
                } catch (f) {
...
function WithSuiteNavData(b, a, d) {
    var c = MakeSuiteNavDataLoader(a, null);
    SuiteNavCommonGetSuiteLinks(c, function(c) {
        var a = null;
        try {
            a = JSON.parse(c);
            a == null
...
var _testKey, _testValue;

function TestStorage(a) {
    var b = false;
    a.setItem(_testKey, _testValue);
    if (a.getItem(_testKey) === _testValue) b = true;
    a.removeItem(_testKey);
    return b
}
var _local, _session, c_strUndefined, key_Date, key_Language, key_LinksJson, key_MySiteLinks, key_SuiteNavHeight, key_UserKey, key_LinksCached;

function GetPageContextInfoValueOrNull(b) {
    var c = null,
        a;
    if (typeof _spPageContextInfo !== c_strUndefined && Boolean(a = _spPageContextInfo) && typeof a[b] !== c_strUndefined) c = String(a[b]);
    return c
}
...
function SuiteNavCommonGetCurrentUICultureOrNone() {
    var a = GetPageContextInfoValueOrNull("currentUICultureName");
    if (!Boolean(a)) a = "none";
    return a
}
...
function GetCachedJsonIfValid() {
    ...
    if (SuiteNavCommonGetCurrentUserKey() == a.getItem(key_UserKey) && SuiteNavCommonGetCurrentUICultureOrNone() == a.getItem(key_Language)) {
        var b = a.getItem(key_LinksJson);
        if (typeof b == "string") c = b
    }
    return c
}

function ExtractSuiteMetadata(b) {
    var a = b;
    try {
...
function ExtractSuiteProperty(b, c) {
    try {
        var a = JSON.parse(b);
...
function ExtractSuiteNavBarData(a) {
    if (a != null) {
...
function SuiteNavCommonGetSuiteLinks(d, a, c, g, h) {
    if (typeof c != "number") c = 1;
    var i = c > 1,
        k = null,
        b = !h ? GetCachedJsonIfValid() : null,
        e = false;
    if (b != null) {
        var f = Date.parse(String(BrowserStorage.local.getItem(key_Date))),
            j = 24 * 60 * 60 * 1e3;
        e = !(f > 0) || +new Date - f > j;
        if (c > 1 && typeof BrowserStorage.session.getItem(key_LinksCached) != "string") e = true
    }
    if (typeof SP !== c_strUndefined && typeof SP._yam !== c_strUndefined)
        if (!e && Boolean(b) && b.indexOf(Boolean(SP._yam) ? "ShellNewsfeed" : "ShellYammer") !== -1) e = true;
    if (b == null) {
        if (d != null) d(function(b) {
            SuiteNavCommonCacheSuiteLinks(b);
            a != null && a(b)
        });
        else a != null && a(null);
        return
    }
    if (c != ExtractSuiteVersion(b) || Boolean(g) != ExtractIsMobile(b))
        if (Boolean(d) && (typeof SuiteNavCommonCacheSuiteLinks == c_strUndefined || typeof SuiteNavCommonCacheSuiteLinks._reloadingData != "boolean" || Boolean(SuiteNavCommonCacheSuiteLinks._reloadingData))) {
            SuiteNavCommonCacheSuiteLinks._reloadingData = true;
            SuiteNavCommonClearSuiteLinksCache();
            SuiteNavCommonGetSuiteLinks(d, a, c, g);
            return
        } if (e) d != null && d(function(d) {
        if (SuiteNavCommonCacheSuiteLinks(d)) b = d;
        a != null && c > 1 && a(i ? d : ExtractSuiteLinks(d))
    });
    a != null && a(b)
}

function SuiteNavCommonCacheSuiteLinks(c) {
    var f = 0;
    SuiteNavCommonCacheSuiteLinks._reloadingData = false;
    try {
        var a = JSON.parse(c);
        if (a != null && a.NavBarData != null && a.NavBarData.Dimensions != null) {
            var d = Number(a.NavBarData.Dimensions.Top);
            d > 0 && BrowserStorage.local.setItem(key_SuiteNavHeight, String(d) + "px")
        }
        if (a != null && a.DoNotCache === true) return false
    } catch (g) {
        return false
    }
    var e = SuiteNavCommonGetCurrentUICultureOrNone(),
        b = BrowserStorage.local;
    b.setItem(key_LinksJson, c);
    b.setItem(key_Language, e);
    b.setItem(key_Date, new Date);
    b.setItem(key_UserKey, SuiteNavCommonGetCurrentUserKey());
    BrowserStorage.session.setItem(key_LinksCached, true);
    return true
}

function SuiteNavCommonClearSuiteLinksCache(c) {
    var a = [key_LinksJson, key_Language, key_Date, key_UserKey, key_SuiteNavHeight];
    !c && a.push(key_MySiteLinks);
    for (var b = 0; b < a.length; b++) BrowserStorage.local.removeItem(a[b]);
    BrowserStorage.session.removeItem(key_LinksCached)
}

function RequestHeader(a, b) {
    this.header = a;
    this.value = b
}
var _restHeaders;

function WithRestValue(a, b) {
    WithUrlContents(a, b, _restHeaders)
}

function WithUrlContents(c, d, b) {
    WithLoadedXMLHttpRequest(a, d, b);
...
function WithLoadedXMLHttpRequest(e, f, b) {
...
function AjaxNavigate$add_navigate(a) {
    ...
    var b = Array.indexOf(this._list, a, 0); - 1 == b && this._list.push(a)
}

function AjaxNavigate$remove_navigate(a) {
    if ("function" != typeof a) throw "unexpected";
    var b = Array.indexOf(this._list, a, 0); - 1 != b && this._list.splice(b, 1)
}
...
function AjaxNavigate$_UrlFromHashBag(b) {
    var a = b.url,
        d = b.anchorTag;
    for (var c in b)
        if (c != "anchorTag" && c != "url") a += "#" + c + "=" + encodeURIComponent(b[c]);
    if (Boolean(d)) a += "#" + d;
...
function AjaxNavigate$_raiseNavigate(d) {
    for (var b = this._buildHashBag(AjaxNavigate$_GetWindowLocationHash(window.location.href)), c = this._list.length, a = 0, e = c; a < e; a++) this._list[a](d, b)
}
...
function AjaxNavigate$convertMDSURLtoRegularURL(a) {
    var c = a;
    if (a != null || a != "") {
        var f = "/_layouts/15/start.aspx",
            b = a.toLowerCase().indexOf(f);
        if (b != -1) {
...
function AjaxNavigate$update(h, f, k, b) {
    var j = AjaxNavigate$_GetWindowLocationHash(window.location.href),
        d = {},
        a = "";
    if (null == h) {
        if (j != null && j.length != 0) {
            var e = j.substr(1).split("#"),
                m = e.length,
                l = 0;
    ...
    for (c in d)
        if (c != "anchorTag") a += "#" + c + "=" + encodeURIComponent(d[c]);
    if (null != h) {
        var p = _dlgWndTop();
    ...
    if ("undefined" != typeof k && k || !i)
        ...
        if (i) {
            ...
            var o = AjaxNavigate$_GetWindowLocationNoHash(window.location.href),
                n = GetUrlFromMDSLocation(o, RemoveMDSQueryParametersFromUrl(a), true);
